"""
Taken from: https://github.com/kaland313/Duckietown-RL/blob/master/duckietown_utils/wrappers/reward_wrappers.py
...
class NotInLaneReward(gym.RewardWrapper):
    ...
    def reward(self, reward):
        try:
            self.unwrapped.get_lane_pos2(self.unwrapped.cur_pos, self.unwrapped.cur_angle)
        except NotInLane:
            return reward - 10
        return reward
    
class LateralVelocityReward(gym.RewardWrapper):
    def __init__(self, env: gym.Env):
        super().__init__(env)
    
    def reward(self, reward):
        return reward + np.mean(self.unwrapped.wheelVels)
